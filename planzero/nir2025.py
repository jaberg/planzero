    ...
    def on_add_project(self, state):
        ...
        for (sector, ghg, pt), offset in co2e_pt.ravel_keys_offsets():
            ...
            if not isinstance(obj, STS):
                continue
            ts = obj
            assert ts.t_unit == u.year
            assert ts.v_unit == u.kt_CO2e
            ...
            state.declare_sts(project=self, sts=ef_ts, name=name, write=True)
...
def foo_prediction_algo(train_valid_thresh_year, horizon):
    ...
    state.add_project(Foo(last_training_year=train_valid_thresh_year))
