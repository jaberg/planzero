...
def sts_a6_1_2():
    ...
    rval = {}
    for geo in Geo:
        if geo in (Geo.CA, Geo.PX): # not present
            continue
        values = _repeat_prev_for_omitted_years(
                in_times,
...
class Est_Natural_Gas_Used_by_Electricity_Utilities_2005_to_2013(PTValues):
    ...
    def __init__(self, volume_unit='m3_NG_mk'):
        ...
        if 0:
            # The documentation on Statistics Canada suggested this might be a good 
            # data set to use here, but it has higher m3 values used, and the difference
            # appears to match the volume of gas used by industrial / producers.
            # Utilities and producers are modelled as burning non-marketable natural gas
            # so I'm still a bit confused, but am electing not to use this data
            # set here.
            alt = sc_nir.Archived_Electric_Power_Generation_Annual_Fuel_Consumed_by_Electrical_Utility().ptv_natural_gas()
            for pt in Geo.provinces_and_territories():
                alt.val_d.setdefault(pt, None)
            val_d = alt.val_d
        super().__init__(val_d=val_d)
...
class Est_Natural_Gas_Used_by_Industry_Electricity_Generation_2005_to_2013(PTValues):
    ...
    def __init__(self, volume_unit='m3_NG_nmk'):
        ...
        val_d = {}
        for pt in Geo.provinces_and_territories():
            if pt in (Geo.BC, Geo.SK, Geo.MB, Geo.QC, Geo.NS, Geo.NB, Geo.PE, Geo.YT, Geo.NU, Geo.PX):
                assert pt not in gen_by_geo
                val_d[pt] = None
                continue
            # how much power did they produce
